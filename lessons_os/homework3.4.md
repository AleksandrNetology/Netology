...
"Нас это огорчит, но не остановит" (с)
...
В интернете удалось найти следующий ответ:

`nr_open` - максимальный лимит пользователя на открытые дескрипторы ядра.
...
`file-max` - максимальное значение дескрипторов этой версии ОС.
Значение `file-max`:
```sh
...
Далее, мягкий лимит на файловые дескрипторы пользователя:
```sh
...
А вот жёсткий лимит на файловые дескрипторы пользователя:
```sh
