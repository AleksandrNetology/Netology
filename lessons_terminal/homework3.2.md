...
>5. Получится ли одновременно передать команде файл на `stdin` и вывести ее `stdout` в другой файл?
	...
	--- Выполнение -----------------------------------------------------------------------
	vagrant@netology:~$ cat test/f1.txt 			# Смотрим содержимое исходного файла
	1 - uno
	2 - due
	3 - tre
	4 - quattro
...
>9. Что выведет команда cat `/proc/$$/environ`? Как еще можно получить аналогичный по содержанию вывод?
	...
	--- Выполнение -----------------------------------------------------------------------
	Команда cat /proc/$$/environ выводит все переменные окружения.
	То же самое делают команды: "printenv", "env" и "export", но переменные окружения выводятся построчно.
	--------------------------------------------------------------------------------------
...
>13. Бывает, что есть необходимость переместить запущенный процесс из одной сессии в другую.
	...
	--- Выполнение -----------------------------------------------------------------------
	vagrant@netology:~$ sudo apt install reptyr
	vagrant@netology:~$ screen -v
	Screen version 4.08.00 (GNU) 05-Feb-20
	vagrant@netology:~$ screen -S test_task1
	vagrant@netology:~$ screen -S test_task2
	vagrant@netology:~$ screen -ls
	There is a screen on:
        1623.test_task2 (02/03/2022 02:39:16 PM)        (Detached)
        1514.test_task1 (02/03/2022 02:20:00 PM)        (Detached)
	2 Sockets in /run/screen/S-vagrant.
	
Да, кстати, надо исправить значение: kernel.yama.ptrace_scope = 0
	
	Теоретически надо сделать так: reptyr - l PID , где PID - это какой-то фоновый процесс, который и надо закинуть
	в screen. Как это сделать практически - ХЗ. Не смог найти решения этой загадки. Примеров ноль, улитита глючит, виснет наглухо.
	10 раз перезапускашь ssh сессию, чтоб убить эту заразу.
	
	Почему объяснение работы reptyr и примеры с ней не включить в лекцию?
	--------------------------------------------------------------------------------------
...
>14. `sudo echo string > /root/new_file` не даст выполнить перенаправление под обычным пользователем, т.к.
	...
	из stdout команды echo. Так как команда tee запущена от sudo, то она имеет права на запись в new_file
	--------------------------------------------------------------------------------------
